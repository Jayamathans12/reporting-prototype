  ...
  return (
    <aside className="flex h-full flex-col rounded-sm border border-chef-line bg-chef-surface p-4">
      ...
      <div className="mt-3 flex items-center gap-2">
        ...
        <button type="button" onClick={() => setStatusFilter("Passed")} className={counterClass(statusFilter === "Passed")}>
          <StatusIcon status="Passed" className="h-4 w-4" />
          <span className="font-semibold text-chef-text">{counts.passed}</span>
        </button>
        <button type="button" onClick={() => setStatusFilter("Failed")} className={counterClass(statusFilter === "Failed")}>
      ...
      <ul className="mt-3 min-h-0 flex-1 space-y-2 overflow-y-auto pr-1">
          ...
          <li key={entry.timestamp}>
            <button
              ...
              className={`flex w-full items-start gap-2.5 rounded-sm border px-3 py-2.5 text-left transition-colors ${
                entry.timestamp === activeTimestamp
        ...
        {visible.length === 0 && (
          <li className="py-6 text-center text-[13px] text-chef-text-muted">No scans in this range.</li>
        )}
      </ul>
    </aside>
  );
